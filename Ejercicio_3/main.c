...
void color_imprimir(const color_t c) {
    printf("%d ", (unsigned int)(c.r > 1 ? 255 : (c.r * 255)));
    printf("%d ", (unsigned int)(c.g > 1 ? 255 : (c.g * 255)));
    printf("%d ", (unsigned int)(c.b > 1 ? 255 : (c.b * 255)));
}
...
color_t computar_intensidad(const arreglo_t *esferas, const arreglo_t *luces, const color_t ambiente, const vector_t o, const vector_t d) {
    ...
    if (t_min != INFINITO) {
        ...
        return color_sumar(c_difusa, ambiente, e_prueba->ka);
    }
    return NEGRO;
}
